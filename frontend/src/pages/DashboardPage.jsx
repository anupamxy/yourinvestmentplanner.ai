...
export default function DashboardPage() {
  ...
  return (
    <Layout>
      {/* Welcome banner */}
      <div className="bg-gradient-to-r from-blue-600 to-blue-800 dark:from-blue-800 dark:to-slate-900 rounded-2xl p-6 text-white mb-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-100">Recent Reports</h2>
            {reports.length > 0 && (
              <span className="text-xs text-slate-400 dark:text-slate-500">{reports.length} total</span>
            )}
          </div>
          ...
          {loadingReports ? (
            ...
            <div className="card text-center py-12">
              ...
              <p className="text-slate-400 dark:text-slate-500 text-sm mt-1 mb-4">Run your first analysis to get personalized investment insights</p>
        ...
        <div className="space-y-4">
            ...
            {profile ? (
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium capitalize text-slate-700 dark:text-slate-300">{profile.risk_tolerance}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium capitalize text-slate-700 dark:text-slate-300">{profile.time_horizon}-term</span>
                </div>
                <div className="mt-3">
                  <span className="text-slate-500 dark:text-slate-400 block mb-1">Sectors</span>
                  <div className="flex flex-wrap gap-1">
                    {profile.sectors.map((s) => (
                      <span key={s} className="text-xs bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded-full capitalize">
                        {s.replace('_', ' ')}
                      </span>
                    ))}
                  </div>
              ...
              <div className="text-center py-4">
                <p className="text-sm text-slate-500 dark:text-slate-400 mb-3">No profile yet</p>
          ...
          <div className="card">
            ...
            <div className="space-y-2">
              <Link to="/run" className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 transition-colors text-sm text-slate-700 dark:text-slate-300">
                <Play size={16} className="text-blue-600" /> Run Analysis
              </Link>
              <Link to="/preferences" className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 transition-colors text-sm text-slate-700 dark:text-slate-300">
                <Settings size={16} className="text-purple-600" /> Update Preferences
              </Link>
              <Link to="/history" className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 transition-colors text-sm text-slate-700 dark:text-slate-300">
